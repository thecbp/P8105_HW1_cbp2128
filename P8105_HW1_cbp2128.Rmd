...
```
results in:
```{r p1_mean_attempts_display, echo = FALSE }
kable(means_df)
```
...
```{r p1_explicit_coercion_2_display, echo = FALSE}
...
All of the color scales can end up looking the same since we're only looking at binary sets of values (TRUE/FALSE, 0/1 for logical, 1/2 for factor). The resulting plots only have two colors, but the numeric scale gives the possibility of a gradient.

```{r save_p2_plot_1, include = FALSE }
# Save the logical plot for HW1
